<script lang="ts">
	...
	import type { PageData } from './$types'; // Removed because module does not exist
...
<div class="container">
	...
	<div class="sidebar">
		...
		<div class="tabs">
			...
			<button class:active={activeTab === 'nearby'} on:click={() => (activeTab = 'nearby')}>
					...
					{:else}
						{#each myGroups as membership}
							<div
								class="group-card"
								on:click={() => {
									selectedGroup = membership.group;
									activeTab = 'nearby';
								}}
							>
			...
			{:else if activeTab === 'nearby'}
				...
				{:else}
					<div class="section">
						<div class="section-header">
							<h3>Nearby Places</h3>
							<button class="create-btn" on:click={() => (showCreatePlace = true)}>
								+ Create Place
							</button>
						</div>
						...
						{#if showCreatePlace}
							<div class="create-form">
								<input type="text" placeholder="Place name" bind:value={newPlace.name} />
								<select bind:value={newPlace.place_type}>
								...
								<label>
									Radius (meters): {newPlace.radius}m
									<input type="range" min="10" max="500" bind:value={newPlace.radius} />
								</label>
								...
								<div class="form-actions">
									<button on:click={handleCreatePlace}>Create at my location</button>
									<button on:click={() => (showCreatePlace = false)}>Cancel</button>
								</div>
							</div>
						{/if}

						{#each nearbyPlaces as place}
							<div class="place-card" on:click={() => selectPlace(place.place_id)}>
								<div class="place-info">
									<strong>{place.place_name}</strong>
									<span class="distance">{Math.round(place.distance_meters)}m away</span>
									{#if place.active_members > 0}
										<span class="active-count">
											{place.active_members} active
										</span>
									{/if}
								</div>
							</div>
